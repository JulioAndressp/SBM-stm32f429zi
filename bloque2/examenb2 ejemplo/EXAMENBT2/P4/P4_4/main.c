...
int main(void)
{
  ...
  reset_lcd();
	...
	LCD_symbolToLocalBuffer_L1('S');
	LCD_symbolToLocalBuffer_L1('P');
	LCD_symbolToLocalBuffer_L1('A');
	LCD_symbolToLocalBuffer_L1('Ñ');
	LCD_symbolToLocalBuffer_L1('I');
	LCD_symbolToLocalBuffer_L1('T');
	LCD_symbolToLocalBuffer_L1('A');

	//sprintf(buffer, "Prueba valor1: %d",number1);
	
	
	
	//sprintf(buffer, "Prueba valor2: %f",number2);
	LCD_update();
...
void LCD_symbolToLocalBuffer_L2(uint8_t symbol)
{
  ...
  for (i = 0; i<12; i++){ //Vamos a escribir las 12 primeras columnas de la letra
     ...
     buffer [i+256 + positionL2] = valor1; //pagina 0 escribimos los valores impares de arial
     buffer [i+384 + positionL2] = valor2; //pagina 1 escribimos los valores pares de arial
