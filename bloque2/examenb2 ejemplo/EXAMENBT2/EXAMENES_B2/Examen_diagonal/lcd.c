...
//	int desplazamiento=0;
	//Borrado_LCD();//Para cada vez que pulso por cuadrantes lo quito
	
	for( i = 0; i < 4 ;i++)
	{
		for( j = 0; j < 8;j++)
		{ 
		  if ( posicion == 3)//i en vez de posicion si quiero que se haga solo del tiron
			{
			buffer[4*j + 480] = 1 << j;
			buffer[4*j+1 + 480] = 1 << j;
			buffer[4*j+2 + 480] = 1 << j;
			buffer[4*j+3 + 480] = 1 << j;
			}
			else if ( posicion == 2)
			{
			buffer[4*j + 320] = 1 << j;
			buffer[4*j+1 + 320] = 1 << j;
			buffer[4*j+2 + 320] = 1 << j;
			buffer[4*j+3 + 320] = 1 << j;
			}
			else if ( posicion == 1)
			{
			buffer[4*j + 160] = 1 << j;
			buffer[4*j+1 + 160] = 1 << j;
			buffer[4*j+2 + 160] = 1 << j;
			buffer[4*j+3 + 160] = 1 << j;
			}
			else
			{
			buffer[4*j] = 1 << j;
