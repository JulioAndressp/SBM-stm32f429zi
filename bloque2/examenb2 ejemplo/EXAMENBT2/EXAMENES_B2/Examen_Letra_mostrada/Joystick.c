...
static void Timer1_Callback (void const *arg) 
{
 ...
 static void Timer2_Callback (void const *arg) {
	 if(inverso)
		...
		{
				Borrado_LCD();
				LCD_EscribeFrase("Letra mostrada: A",1);
				LCD_update();
		}
		if(letras==1)
		{
				Borrado_LCD();
				LCD_EscribeFrase("Letra mostrada: E",1);
				LCD_update();
		}	
		if(letras==2)
		{
				Borrado_LCD();
				LCD_EscribeFrase("Letra mostrada: I",1);
				LCD_update();
		}
		if(letras==3)
		{
				Borrado_LCD();
				LCD_EscribeFrase("Letra mostrada: O",1);
				LCD_update();
		}	
		if(letras==4)
		{
				Borrado_LCD();
			LCD_EscribeFrase("Letra mostrada: U",1);
				LCD_update();
		}
...
int Timer_Joystick (void) {
  ...
  exec2 = 2U;
  tim_digitos = osTimerNew((osTimerFunc_t)&Timer2_Callback, osTimerOnce, &exec2, NULL);
