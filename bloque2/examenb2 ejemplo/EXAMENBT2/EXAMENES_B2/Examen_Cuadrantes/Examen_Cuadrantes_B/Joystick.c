...
static void Timer1_Callback (void const *arg) 
{
  ...
  {
		cadencia = cadencia+200;
		Borrado_LCD();
		sprintf(frase,"Nueva cadencia:%d",cadencia);
  ...
  {
			if(aut_on==true)//Encendidio activo
			{
				osTimerDelete(tim_cuadrantes);

			}else if (aut_on==false)
			{
			Timer_Joystick();
			}
			aut_on =!aut_on;
 ...
 static void Timer2_Callback (void const *arg) {
		...
		osTimerStart(tim_cadencia,cadencia);
