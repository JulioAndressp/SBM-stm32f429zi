...
void Borrado_LCD(void)
{
	...
	//Pintar barra de un pix
	void Pintar_barra(int posicion,bool primeras_paginas)
	{
		int columnas ;
		int filas ;
		Borrado_LCD();
		//Posicion y j es la fila 
		//i es la columna
		for(filas = 0; filas < posicion;filas++)
		{
			for(columnas = 0; columnas < 128 ; columnas ++)
			{
					if(pagina == 3)
					buffer[columnas+384]= 1<<filas;
					else if (pagina == 2)
						buffer[columnas+256]=1<<filas;
					else if(pagina == 1)
						buffer[columnas+128]=1<<filas;
					else 
						buffer[columnas]=1<<filas;
			}
		}
