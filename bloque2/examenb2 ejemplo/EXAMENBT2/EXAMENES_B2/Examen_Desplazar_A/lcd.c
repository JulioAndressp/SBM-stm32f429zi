...
void LCD_EscribeFraseAmbLineas(char *frase, uint8_t linea)
{
  uint8_t i;
...
void Borrado_LCD(void)
{
	...
	void pintar_pelota(int posicion)
	{
		...
		for( i =posicion;i<8+posicion;i++)
		{ 
			buffer[i+posicion]=0xFF;
		}
		for( i = posicion+128;i<136+posicion;i++)
		{ 
			buffer[i+posicion]=0x0F;
		}
	...
	void Desplazar_A(int pulsacion,int linea)
	{
		if(pulsacion==0)
		{
				Borrado_LCD();
			  sprintf(frase,"A_______________");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==1)
		{
				Borrado_LCD();
			  sprintf(frase,"_A______________");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==2)
		{
				Borrado_LCD();
			  sprintf(frase,"__A_____________");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==3)
		{
				Borrado_LCD();
			  sprintf(frase,"____A___________");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==4)
		{
				Borrado_LCD();
			  sprintf(frase,"_____A__________");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==5)
		{
				Borrado_LCD();
			  sprintf(frase,"______A_________");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==6)
		{
				Borrado_LCD();
			  sprintf(frase,"_______A________");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==7)
		{
				Borrado_LCD();
			  sprintf(frase,"________A_______");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==8)
		{
				Borrado_LCD();
			  sprintf(frase,"_________A______");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==9)
		{
				Borrado_LCD();
			  sprintf(frase,"__________A_____");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==10)
		{
				Borrado_LCD();
			  sprintf(frase,"___________A____");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==11)
		{
				Borrado_LCD();
			  sprintf(frase,"____________A____");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==12)
		{
				Borrado_LCD();
			  sprintf(frase,"_____________A___");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==13)
		{
				Borrado_LCD();
			  sprintf(frase,"______________A__");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==14)
		{
				Borrado_LCD();
			  sprintf(frase,"________________A_");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
				if(pulsacion==15)
		{
				Borrado_LCD();
			  sprintf(frase,"_________________A");
				LCD_EscribeFrase(frase,linea);
				LCD_update();
		}
