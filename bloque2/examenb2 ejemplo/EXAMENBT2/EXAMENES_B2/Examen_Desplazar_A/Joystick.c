...
static void Timer1_Callback (void const *arg) 
{
    ...
    if(HAL_GPIO_ReadPin(PUERTO_JOYSTICK_UP_RIGHT_GPIOB,JOYSTICK_RIGHT)== GPIO_PIN_SET)
  {
		...
		pulsacion++;
		Desplazar_A(pulsacion,linea);
		osTimerStart(tim_desplazo,3000);
...
int Timer_Joystick (void) {
  ...
  exec2 = 2U;
  tim_desplazo = osTimerNew((osTimerFunc_t)&Timer2_Callback, osTimerOnce, &exec2, NULL);
  if (tim_desplazo != NULL) {  // One-shot timer created
    // start timer with delay 50ms
    osTimerStart(tim_desplazo,3000);
    if (status != osOK) {
      return -1;
	  ...
	  exec3 = 3U;
  tim_led = osTimerNew((osTimerFunc_t)&Timer3_Callback, osTimerOnce, &exec3, NULL);
  if (tim_led != NULL) {  // One-shot timer created
    // start timer with delay 50ms
    osTimerStart(tim_led,2000);
    if (status != osOK) {
      return -1;
    }
	}		
