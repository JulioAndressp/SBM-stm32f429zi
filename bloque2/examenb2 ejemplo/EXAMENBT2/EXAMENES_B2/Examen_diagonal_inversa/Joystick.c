...
int Timer_Joystick (void) {
  ...
  exec2 = 2U;
  tim_diagonal = osTimerNew((osTimerFunc_t)&Timer2_Callback, osTimerOnce, &exec2, NULL);
  if (tim_diagonal != NULL) {  // One-shot timer created
    // start timer with delay 50ms
    if (status != osOK) {
      return -1;
    }
