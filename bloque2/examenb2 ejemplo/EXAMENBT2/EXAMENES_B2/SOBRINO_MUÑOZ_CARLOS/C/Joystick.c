...
int Timer_Joystick (void) {
  ...
  tim_bloqueado = osTimerNew((osTimerFunc_t)&Timer2_Callback, osTimerOnce, &exec2, NULL);
  if (tim_bloqueado != NULL) {  // One-shot timer created
    if (status != osOK) {
      return -1;
    }
	}	
...
int Init_ThJoystick (void) {
 ...
 void ThAzul (void *argument) {
  ...
  osTimerStart(tim_bloqueado,5000);
  osThreadYield();
  }
}
