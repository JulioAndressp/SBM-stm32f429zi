 ...
 static void Timer2_Callback (void const *arg) 
{
	if(posicion==64)
	{
		pintar_pelota(posicion);
		/*------------APARTADO B-------------*/
		sprintf(frase,"Puntuacion: %d",puntuacion);
		LCD_EscribeFrase(frase,2);
		/*------------APARTADO B-------------*/
		LCD_update();
		posicion=0;
	}
	...
	osTimerStart(tim_pelota,velocidad);
...
int Timer_Joystick (void) {
  ...
  exec2 = 2U;
  tim_pelota = osTimerNew((osTimerFunc_t)&Timer2_Callback, osTimerOnce, &exec2, NULL);
  if (tim_pelota != NULL) {  // One-shot timer created
    // start timer with delay 50ms
    osTimerStart(tim_pelota,velocidad);
    if (status != osOK) {
      return -1;
