...
void pintar_diagonal(int posicion)
{
	...
	for(j=0;j<13;j++)//Paginas
	{
	for(i=0;i<8;i++)//Filas
	{
		buffer[4*i+desplazamiento]  = 1<<i;//Primer cuadrado
		buffer[4*i+1+desplazamiento]=1<<i;//Segundo cuadrado
		buffer[4*i+2+desplazamiento]=1<<i;//Tercer cuadrado
		buffer[4*i+3+desplazamiento]=1<<i;//Cuarto cuadrado
	}
	desplazamiento=128+32*j;
	LCD_update();
}
...
void BarridoVertical(void){
	...
	Borrado_LCD();
			...
			buffer[linx+128] = 0xFF;
			
			buffer[linx+256] = 0xFF;
			
			buffer[linx+384] = 0xFF;
...
void pintar_pelota(void)
    ...
    int seg=0;
		//Borrado_LCD();
   if(pelota<64){
		for( prim =pelota;prim<8+pelota;prim++)
		{ 
			buffer[prim+pelota]=0xFF;
      buffer[256+prim+pelota]=0xFF;
		}
    pelota=pelota+8;
  }
...
void pintar_pelotaIM(void)
    ...
    int seg=0;
		//Borrado_LCD();
   if(pelota2<64){
		for( seg =pelota2;seg<8+pelota2;seg++)
		{ 
			buffer[128+seg+pelota2]=0xFF;
      buffer[384+seg+pelota2]=0xFF;
		}
    pelota2=pelota2+8;
  }
	...
	void pintarLCDEntero (void){
    
    int borr;
    for( borr=0;borr<128;borr++)
    {
      buffer[borr]=0xFF;
    }
    for( borr=128;borr<256;borr++)
    {
      buffer[borr]=0xFF;
    }
    for( borr=256;borr<384;borr++)
    {
      buffer[borr]=0xFF;
    }
    for( borr=384;borr<512;borr++)
    {
      buffer[borr]=0xFF;
    }
    LCD_update();
	...
	void Escribe2Enteros(char cadena[], uint32_t entero1, uint32_t entero2, int linea){
  char string [50];
  char cadenaEntero[32];
  
  sprintf(cadenaEntero, "%02d:%02d", entero1,entero2);
  strcpy(string,cadena);
  strcat(string,cadenaEntero);
  LCD_EscribeFrase(string,linea);
  //positionL2=0;
}
...
//void escribir_arroba (uint8_t linea){
//	
//	
//			pos++;
