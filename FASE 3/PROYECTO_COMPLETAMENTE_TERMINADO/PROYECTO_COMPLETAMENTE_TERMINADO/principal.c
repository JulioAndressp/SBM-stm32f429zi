...
extern osMessageQueueId_t mid_MsgQueue_RDA_pp_com;
	...
	Init_Timers_Clock();
		...
		while (1) {
			...
			if(MODO == SELECTOR){
				...
				status_com = osMessageQueueGet(mid_MsgQueue_RDA_pp_com, &msg_RDA_pp_com,NULL,0U);
				
				if (status_com == osOK){
					
					memcpy(msg_com.RDA_REG_WRITE,msg_RDA_pp_com.RDA_REG_WRITE,sizeof(msg_RDA_pp_com.RDA_REG_WRITE));
					osMessageQueuePut(mid_COM_Queue,&msg_com,NULL,0U);
				...
				if(MODO_SELECTOR == SELECTOR_MANUAL)	{
					...
					if(status_joy == osOK) { 
						...
						//		memcpy(msg_com.RDA_REG_WRITE,msg_RDA_pp.RDA_REG_WRITE,sizeof(msg_RDA_pp.RDA_REG_WRITE));
							//	osMessageQueuePut(mid_COM_Queue,&msg_com,NULL,0U);
							}
