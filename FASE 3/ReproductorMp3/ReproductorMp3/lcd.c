...
void ThLCD (void *argument) {
  ...
  Borrado_LCD();
	...
	while (1) {
			status = osMessageQueueGet(mid_JoystickLCD, &msglcd, NULL, osWaitForever);
		if(status == osOK){
			if (msglcd.Idx == 0U)//Modo Reposo
			{
				Borrado_LCD();
				sprintf(frase,"SBM 2021 T: %2.1f C",temperatura);
				LCD_EscribeFrase(frase,1);
				sprintf(frase2, "     %.2d:%.2d:%.2d ",horas,minutos,segundos);
				LCD_EscribeFrase(frase2,2);
					
		}
			if (msglcd.Idx == 1U)//Modo Reproduccion
			{
				Borrado_LCD();
				sprintf(frase,"F: %.2d C: %.2d Vol: %.2d ",actual_folder,actual_song,msgvol.Buf[20]);
				LCD_EscribeFrase(frase,1);
				sprintf(frase2, "     Trepr: %.2d:%.2d ",min_song,seg_song);
				LCD_EscribeFrase(frase2,2);
